...
from .models import ModaModel, ModaMembers, ModaBackNForthChat


class ModaBackNForthChatConsumer(AsyncWebsocketConsumer):
    """
    WebSocket consumer for BackNForth chat system in S-Links (Moda)
    Handles real-time messaging for S-Link members
    """
    
    async def connect(self):
        """Handle WebSocket connection"""
        self.moda_code = self.scope['url_route']['kwargs']['moda_code']
        self.room_group_name = f'moda_backnforth_chat_{self.moda_code}'
        
        # Verify user is authenticated and is a member of the S-Link
    ...
    def is_user_member(self):
        """Check if user is a member of the S-Link"""
        try:
            ...
            print(f"Looking for Moda with code: {self.moda_code}")
            moda = ModaModel.objects.get(code=self.moda_code)
            print(f"Found Moda: {moda}")
            
            is_member = ModaMembers.objects.filter(
                user=user, 
                moda=moda, 
                is_member=True
            ).exists()
            ...
            print(f"ModaModel with code {self.moda_code} does not exist")
            return False
    ...
    def save_message(self, message_content, reply_to_id=None):
        ...
        try:
            ...
            moda = ModaModel.objects.get(code=self.moda_code)
            print(f"Moda found: {moda.code}")
            
            reply_to_message = None
            if reply_to_id:
                try:
                    reply_to_message = ModaBackNForthChat.objects.get(
                        id=reply_to_id, 
                        moda=moda
                    )
                except ModaBackNForthChat.DoesNotExist:
                    print(f"Reply message with id {reply_to_id} not found")
                    pass
            
            message = ModaBackNForthChat.objects.create(
                moda=moda,
                sender=user,
    ...
    def edit_message(self, message_id, new_content):
        ...
        try:
            ...
            message = ModaBackNForthChat.objects.get(
                id=message_id,
                moda=moda,
                sender=user,
                is_deleted=False
        ...
        except (ModaBackNForthChat.DoesNotExist, Exception):
            return False
    ...
    def delete_message(self, message_id):
        ...
        try:
            ...
            # Allow deletion by sender or S-Link delegate
