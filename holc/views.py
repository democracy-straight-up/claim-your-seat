...
class HolcMembersViewSet(viewsets.ModelViewSet):
    ...
    def vote_admission(self, request, pk=None):
        user = request.user
        ...
        pending_membership = self.get_object()
    ...
    @action(detail=True, methods=["POST"])
    def vote_holc_replacement(self, request, pk=None):
        user = request.user
        ...
        candidate_membership = self.get_object()

        try:
            result = cast_holc_replacement_vote(
                voter=user,
