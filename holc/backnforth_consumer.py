...
from .models import HolcModel, HolcMembers, HolcBackNForthChat


class HolcBackNForthChatConsumer(AsyncWebsocketConsumer):
    """
    WebSocket consumer for BackNForth chat system in HoLC (House of Local Councils)
    Handles real-time messaging for HoLC members
    """
    
    async def connect(self):
        """Handle WebSocket connection"""
        self.holc_code = self.scope['url_route']['kwargs']['holc_code']
        self.room_group_name = f'holc_backnforth_chat_{self.holc_code}'
        
        # Verify user is authenticated and is a member of the HoLC
    ...
    def is_user_member(self):
        """Check if user is a member of the HoLC"""
        try:
            ...
            print(f"Looking for HoLC with code: {self.holc_code}")
            holc = HolcModel.objects.get(code=self.holc_code)
            print(f"Found HoLC: {holc}")
            
            is_member = HolcMembers.objects.filter(
                user=user, 
                holc=holc, 
                is_member=True
            ).exists()
            ...
            print(f"HolcModel with code {self.holc_code} does not exist")
            return False
    ...
    def save_message(self, message_content, reply_to_id=None):
        ...
        try:
            ...
            holc = HolcModel.objects.get(code=self.holc_code)
            print(f"HoLC found: {holc.code}")
            
            reply_to_message = None
            if reply_to_id:
                try:
                    reply_to_message = HolcBackNForthChat.objects.get(
                        id=reply_to_id, 
                        holc=holc
                    )
                except HolcBackNForthChat.DoesNotExist:
                    print(f"Reply message with id {reply_to_id} not found")
                    pass
            
            message = HolcBackNForthChat.objects.create(
                holc=holc,
                sender=user,
    ...
    def edit_message(self, message_id, new_content):
        ...
        try:
            ...
            message = HolcBackNForthChat.objects.get(
                id=message_id,
                holc=holc,
                sender=user,
                is_deleted=False
        ...
        except (HolcBackNForthChat.DoesNotExist, Exception):
            return False
    ...
    def delete_message(self, message_id):
        ...
        try:
            ...
            # Allow deletion by sender or HoLC delegate
